      ...
      <div class="pro-cont-div flex-column flex-space-between flex-align-start">
        <div class="flex-row flex-space-between flex-align-center full-width" style="flex: 1">
          <div class="flex-row flex-just-start flex-align-center">
            <div class="act-tag" v-if="k.market_type === 1">活动</div>
            <div class="product-name">{{k.product_name}}</div>
          </div>
          <q-btn icon="app:iconfenxiang" size="xs" flat round class="share-icon" @click.stop="shareShop" v-if="showShare"></q-btn>
          <!--                <div>抢购时间预留</div>-->
...
<!--          </div>-->
        ...
        <div style="flex: 1">
          <span class="buy-price" v-if="isVip === 1">{{Math.floor((k.member_sale_price - k.dc) * 100) / 100}}元</span>
          <span class="buy-price" v-if="isVip === 0">{{Math.floor((k.sale_price - k.dc) * 100) / 100}}元</span>
          <span class="dc-price">+{{k.dc_value}}贝</span>
        </div>
        <!-----------进度条------------>
